...
namespace cursedearth
{
    ...
    {
        ce_string* string = (ce_string*)ce_alloc_zero(sizeof(ce_string));
    ...
    void ce_string_reserve(ce_string* string, size_t capacity)
    {
        if (capacity > string->capacity) {
            string->str = (char*)ce_realloc(string->str, string->capacity, capacity);
    ...
    void ce_string_assign_va(ce_string* string, const char* fmt, va_list args)
    {
        for (size_t length = 128, limit = 1024; length <= limit; length <<= 1) {
            ...
            int code = vsnprintf(buffer.data(), buffer.size(), fmt, args);
            if (code >= 0 && (size_t)code < buffer.size()) {
                ce_string_assign(string, buffer.data());
                break;
            }
        }
    }
    ...
    size_t ce_string_append_va(ce_string* string, const char* fmt, va_list args)
    {
        for (size_t length = 128, limit = 1024; length <= limit; length <<= 1) {
            ...
            int code = vsnprintf(buffer.data(), buffer.size(), fmt, args);
            if (code >= 0 && (size_t)code < buffer.size()) {
                return ce_string_append(string, buffer.data());
            }
        }
        return 0;
    }
