...
namespace cursedearth
{
    ...
    fs::path find_resource(const std::string& name)
    {
        ...
        throw game_error("video manager", "could not find resource `%1%'", name);
    }
    ...
    video_object_t video_manager_t::make_instance(const std::string& name)
    {
        ...
        if (NULL == mem_file) {
            throw game_error("video manager", "could not open file `%1%'", file_path);
        }

        ce_video_resource* resource = ce_video_resource_new(mem_file);
        if (NULL == resource) {
            ce_mem_file_del(mem_file);
            throw game_error("video manager", "could not create resource `%1%'", file_path);
        }
