...
{
    /**
     * @brief XFree86 VidMode, see xf86vidmode(3) for more details
     */
    typedef struct {
    ...
    int ce_xf86vmmng_calc_rate(XF86VidModeModeInfo* info)
    {
        // dotclock comes in kHz
        return (info->dotclock * 1000.0f) / (info->htotal * info->vtotal) + 0.5f;
    }

    void ce_xf86vmmng_ctor(ce_displaymng* displaymng, va_list args)
    {
        ...
        int mode_count;
        XF86VidModeGetAllModeLines(xf86vmmng->display, XDefaultScreen(xf86vmmng->display), &mode_count, &xf86vmmng->modes);

        for (int i = 0; i < mode_count; ++i) {
            XF86VidModeModeInfo* info = xf86vmmng->modes[i];
            ce_vector_push_back(displaymng->supported_modes, ce_displaymode_new(info->hdisplay, info->vdisplay, bpp, ce_xf86vmmng_calc_rate(info)));
        }
    }

    void ce_xf86vmmng_enter(ce_displaymng* displaymng, size_t index, ce_display_rotation, ce_display_reflection)
    {
        ce_xf86vmmng* xf86vmmng = (ce_xf86vmmng*)displaymng->impl;

        if (NULL != xf86vmmng->modes) {
            XF86VidModeSwitchToMode(xf86vmmng->display, XDefaultScreen(xf86vmmng->display), xf86vmmng->modes[index]);
            XF86VidModeSetViewPort(xf86vmmng->display, XDefaultScreen(xf86vmmng->display), 0, 0);
        }
    }

    void ce_xf86vmmng_exit(ce_displaymng* displaymng)
    {
        ce_xf86vmmng_enter(displaymng, 0, CE_DISPLAY_ROTATION_NONE, CE_DISPLAY_REFLECTION_NONE);
    }

    void ce_xf86vmmng_dtor(ce_displaymng* displaymng)
    {
        ce_xf86vmmng* xf86vmmng = (ce_xf86vmmng*)displaymng->impl;
    ...
    bool ce_xf86vmmng_query(Display* display)
    {
        ...
        if (XF86VidModeQueryExtension(display, &dummy, &dummy) &&
                XF86VidModeQueryVersion(display, &major_version, &minor_version) && major_version == major_version_req && minor_version >= minor_version_req) {
            return true;
        }

        ce_logging_warning("displaymng: XFree86 Video Mode Extension %d.%d found, but %d.%d required", major_version, minor_version, major_version_req, minor_version_req);
        return false;
    }

    /**
     * @brief XRandR, see xrandr(3) for more details
     */
    typedef struct {
        int event_base, error_base;
        int major_version, minor_version;
    ...
    ce_display_rotation ce_xrrmng_xrr2ce_rotation(Rotation xrr_rotation)
    {
        unsigned int rotation = CE_DISPLAY_ROTATION_NONE;
        const ce_display_rotation rotations[] = { CE_DISPLAY_ROTATION_0, CE_DISPLAY_ROTATION_90, CE_DISPLAY_ROTATION_180, CE_DISPLAY_ROTATION_270 };
        const Rotation xrr_rotations[] = { RR_Rotate_0, RR_Rotate_90, RR_Rotate_180, RR_Rotate_270 };
        for (size_t i = 0; i < sizeof(xrr_rotations) / sizeof(xrr_rotations[0]); ++i) {
    ...
    ce_display_reflection ce_xrrmng_xrr2ce_reflection(Rotation xrr_reflection)
    {
        unsigned int reflection = CE_DISPLAY_REFLECTION_NONE;
        const ce_display_reflection reflections[] = { CE_DISPLAY_REFLECTION_X, CE_DISPLAY_REFLECTION_Y };
        const Rotation xrr_reflections[] = { RR_Reflect_X, RR_Reflect_Y };
        for (size_t i = 0; i < sizeof(xrr_reflections) / sizeof(xrr_reflections[0]); ++i) {
    ...
    Rotation ce_xrrmng_ce2xrr_rotation(ce_display_rotation rotation, ce_display_reflection reflection)
    {
    ...
    void ce_xrrmng_ctor(ce_displaymng* displaymng, va_list args)
    {
        ...
        for (int i = 0; i < xrrmng->size_count; ++i) {
            ...
            for (int j = 0; j < rate_count; ++j) {
                ce_vector_push_back(displaymng->supported_modes, ce_displaymode_new(xrrmng->sizes[i].width, xrrmng->sizes[i].height, bpp, rates[j]));
            }
        }
    }

    void ce_xrrmng_enter(ce_displaymng* displaymng, size_t index, ce_display_rotation rotation, ce_display_reflection reflection)
    {
        ce_xrrmng* xrrmng = (ce_xrrmng*)displaymng->impl;
        ...
        for (size_index = xrrmng->size_count - 1; size_index > 0; --size_index) {
            if (mode->width == xrrmng->sizes[size_index].width && mode->height == xrrmng->sizes[size_index].height) {
                break;
            }
        }

        XRRSetScreenConfigAndRate(xrrmng->display, xrrmng->conf, XDefaultRootWindow(xrrmng->display), size_index, ce_xrrmng_ce2xrr_rotation(rotation, reflection), mode->rate, CurrentTime);
    }

    void ce_xrrmng_exit(ce_displaymng* displaymng)
    {
        ce_xrrmng* xrrmng = (ce_xrrmng*)displaymng->impl;

        XRRSetScreenConfigAndRate(xrrmng->display, xrrmng->conf, XDefaultRootWindow(xrrmng->display), xrrmng->orig_size, xrrmng->orig_rotation, xrrmng->orig_rate, CurrentTime);
    }

    void ce_xrrmng_dtor(ce_displaymng* displaymng)
    {
        ce_xrrmng* xrrmng = (ce_xrrmng*)displaymng->impl;
    ...
    bool ce_xrrmng_query(Display* display)
    {
        ...
        ce_logging_warning("displaymng: XRandR Extension %d.%d found, but %d.%d required", major_version, minor_version, major_version_req, minor_version_req);
        return false;
    }
    ...
    ce_displaymng* ce_displaymng_create(Display* display)
    {
        ...
        for (size_t i = 0; i < sizeof(extensions) / sizeof(extensions[0]); ++i) {
            ...
            if (!XQueryExtension(display, extensions[i].name, &dummy, &dummy, &dummy)) {
                ce_logging_warning("displaymng: X11 extension `%s' not supported", extensions[i].name);
                continue;
            }
        ...
        ce_logging_warning("displaymng: no appropriate X11 extensions found, fullscreen mode is not available");
        return NULL;
    }
}
