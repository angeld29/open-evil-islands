...
namespace cursedearth
{
    ...
    void* ce_font_alloc(FT_Memory, long size)
    {
        return ce_alloc_zero(size);
    }

    void ce_font_free(FT_Memory, void* block)
    {
        ce_free_slow(block);
    }

    void* ce_font_realloc(FT_Memory, long cur_size, long new_size, void* block)
    {
    ...
    {
        ce_font* font = (ce_font*)ce_alloc_zero(sizeof(ce_font));
        font->memory.user = font;
        font->memory.alloc = ce_font_alloc;
        font->memory.free = ce_font_free;
        font->memory.realloc = ce_font_realloc;
        ...
        FT_Face face;
        code = FT_New_Memory_Face(font->library, ce_resource_data[resource_data_index], ce_resource_data_sizes[resource_data_index], 0, &face);
        if (0 != code) {
        ...
        assert(face->face_flags & FT_FACE_FLAG_SCALABLE && face->face_flags & FT_FACE_FLAG_HORIZONTAL && "invalid font");
        ...
        unsigned char* image = (unsigned char*)ce_alloc_zero(image_width * image_height);
