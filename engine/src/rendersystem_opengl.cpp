...
void ce_render_system_init(void)
{
    ...
    ce_render_system = ce_alloc_zero(sizeof(struct ce_render_system) +
                                    sizeof(struct ce_opengl_system));

    ce_opengl_system = (struct ce_opengl_system*)ce_render_system->impl;

    ce_render_system->thread_id = ce_thread_self();
    ce_opengl_system->view = CE_MAT4_IDENTITY;
    ...
    glNewList(ce_opengl_system->wire_cube_list, GL_COMPILE);
    ...
    glNewList(ce_opengl_system->solid_cube_list, GL_COMPILE);
    glBegin(GL_QUADS);
    // face 1 front xy plane
    glVertex3f( 1.0f, -1.0f,  1.0f);
    glVertex3f( 1.0f,  1.0f,  1.0f);
...
{
    glCallList(ce_opengl_system->axes_list);
}

void ce_render_system_draw_wire_cube(void)
{
    glCallList(ce_opengl_system->wire_cube_list);
}
...
void ce_render_system_setup_camera(ce_camera* camera)
{
    ...
    ce_opengl_system->view.m[0] = 1.0f - (tyy + tzz);
    ce_opengl_system->view.m[1] = txy + twz;
    ce_opengl_system->view.m[2] = txz - twy;

    ce_opengl_system->view.m[4] = txy - twz;
    ce_opengl_system->view.m[5] = 1.0f - (txx + tzz);
    ce_opengl_system->view.m[6] = tyz + twx;

    ce_opengl_system->view.m[8] = txz + twy;
    ce_opengl_system->view.m[9] = tyz - twx;
    ce_opengl_system->view.m[10] = 1.0f - (txx + tyy);

    ce_opengl_system->view.m[12] = -
        ce_opengl_system->view.m[0] * camera->position.x -
        ce_opengl_system->view.m[4] * camera->position.y -
        ce_opengl_system->view.m[8] * camera->position.z;

    ce_opengl_system->view.m[13] = -
        ce_opengl_system->view.m[1] * camera->position.x -
        ce_opengl_system->view.m[5] * camera->position.y -
        ce_opengl_system->view.m[9] * camera->position.z;

    ce_opengl_system->view.m[14] = -
        ce_opengl_system->view.m[2] * camera->position.x -
        ce_opengl_system->view.m[6] * camera->position.y -
        ce_opengl_system->view.m[10] * camera->position.z;
