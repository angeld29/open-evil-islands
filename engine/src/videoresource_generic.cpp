...
{
    /**
     * @brief Ogg Theora (C) Xiph.Org Foundation
     */
    struct ce_theora
    {
    ...
    size_t ce_theora_size_hint(ce_mem_file*)
    {
        return sizeof(ce_theora);
    }

    void ce_theora_init(ce_theora* theora)
    {
    ...
    void ce_theora_clean(ce_theora* theora)
    {
    ...
    bool ce_theora_pump(ogg_sync_state* sync, ce_mem_file* mem_file)
    {
    ...
    bool ce_theora_test(ce_mem_file* mem_file)
    {
        ce_theora theora;
    ...
    void ce_theora_report_pixelformat(ce_video_resource* video_resource)
    {
    ...
    void ce_theora_report_colorspace(ce_video_resource* video_resource)
    {
        ce_theora* theora = (ce_theora*)video_resource->impl;
    ...
    void ce_theora_report_comments(ce_video_resource* video_resource)
    {
        ce_theora* theora = (ce_theora*)video_resource->impl;
    ...
    bool ce_theora_ctor(ce_video_resource* video_resource)
    {
        ce_theora* theora = (ce_theora*)video_resource->impl;
        ce_theora_init(theora);
        ...
        while (!done) {
            ...
            while (ogg_sync_pageout(&theora->sync, &theora->page) > 0) {
                ...
                // identify the codec
                if(0 == header_count && th_decode_headerin(&theora->info, &theora->comment, &theora->setup, &theora->packet) >= 0) {
                    // it is theora
        ...
        while (0 != header_count && header_count < 3) {
            ...
            if (code > 0) {
                code = th_decode_headerin(&theora->info, &theora->comment, &theora->setup, &theora->packet);
                if (code <= 0) {
                    ce_logging_error("theora: error parsing stream headers");
                    return false;
                }
        ...
        video_resource->fps = (float)theora->info.fps_numerator / theora->info.fps_denominator;
        ...
        if (theora->info.pic_width != theora->info.frame_width || theora->info.pic_height != theora->info.frame_height) {
            ce_logging_debug("theora: frame content is %dx%d with offset (%d,%d)",
                theora->info.frame_width, theora->info.frame_height, theora->info.pic_x, theora->info.pic_y);
        }
    ...
    void ce_theora_dtor(ce_video_resource* video_resource)
    {
        ce_theora* theora = (ce_theora*)video_resource->impl;
        ce_theora_clean(theora);
    }

    bool ce_theora_read(ce_video_resource* video_resource)
    {
        ce_theora* theora = (ce_theora*)video_resource->impl;
        ...
        if (theora->packet.granulepos >= 0) {
            th_decode_ctl(theora->context, TH_DECCTL_SET_GRANPOS, &theora->packet.granulepos, sizeof(theora->packet.granulepos));
        }
    ...
    bool ce_theora_reset(ce_video_resource*)
    {
        // TODO
        return false;
    }
    ...
    /**
     * @brief Bink Video (C) RAD Game Tools, Inc.
    ...
    size_t ce_bink_size_hint(ce_mem_file*)
    {
        return sizeof(ce_bink);
    }

    bool ce_bink_test(ce_mem_file* mem_file)
    {
        ce_bink_header header;
        return ce_bink_header_read(&header, mem_file);
    }

    bool ce_bink_ctor(ce_video_resource* video_resource)
    {
        ...
        bink->data = (uint8_t*)ce_alloc(sizeof(ce_bink_index) * bink->header.frame_count + bink->header.largest_frame_size + FF_INPUT_BUFFER_PADDING_SIZE);
        ...
        if (0 == bink->header.frame_count || 0 == bink->header.video_width || 0 == bink->header.video_height || 0 == bink->header.fps_dividend ||
                0 == bink->header.fps_divider || bink->header.largest_frame_size > bink->header.file_size || bink->header.frame_count > CE_BINK_MAX_FRAMES ||
                bink->header.video_width > CE_BINK_MAX_VIDEO_WIDTH || bink->header.video_height > CE_BINK_MAX_VIDEO_HEIGHT) {
        ...
        ce_logging_debug("bink: video is %ux%u %.02f fps", video_resource->width, video_resource->height, video_resource->fps);
    ...
    void ce_bink_dtor(ce_video_resource* video_resource)
    {
        ce_bink* bink = (ce_bink*)video_resource->impl;
        ce_free(bink->data, sizeof(ce_bink_index) * bink->header.frame_count + bink->header.largest_frame_size + FF_INPUT_BUFFER_PADDING_SIZE);
    ...
    bool ce_bink_read(ce_video_resource* video_resource)
    {
        ce_bink* bink = (ce_bink*)video_resource->impl;
    ...
    bool ce_bink_reset(ce_video_resource* video_resource)
    {
        ce_bink* bink = (ce_bink*)video_resource->impl;
