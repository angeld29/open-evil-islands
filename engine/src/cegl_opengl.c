...
bool ce_gl_init(void)
{
	...
	if (GLEW_OK != error) {
		ce_logging_critical("opengl: glewInit failed: %s", glewGetErrorString(error));
	} else {
		ce_logging_write("opengl: using GLEW %s", glewGetString(GLEW_VERSION));
	}

	ce_gl_context.features[CE_GL_FEATURE_MULTITEXTURE] = GLEW_ARB_multitexture;
	...
	ce_gl_context.features[CE_GL_FEATURE_SHADER_OBJECT] = GLEW_ARB_shader_objects;
	ce_gl_context.features[CE_GL_FEATURE_VERTEX_SHADER] = GLEW_ARB_vertex_shader;
	ce_gl_context.features[CE_GL_FEATURE_FRAGMENT_SHADER] = GLEW_ARB_fragment_shader;
