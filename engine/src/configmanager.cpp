...
namespace cursedearth
{
    ...
    static const char* ce_config_menu_item_names[CE_CONFIG_MENU_TYPE_COUNT][CE_CONFIG_MENU_ITEM_COUNT] = {
        {
            "NewGame",
            "LoadGame",
            "Multiplayer",
    ...
    static bool ce_config_manager_read_light(ce_color section[24], const char* section_name, ce_config_file* config_file)
    {
    ...
    static void ce_config_manager_init_lights(void)
    {
        ...
        for (size_t i = 0; i < CE_CONFIG_LIGHT_COUNT; ++i) {
            ce_path_join(path.data(), path.size(), ce_option_manager->ei_path->str, ce_config_dir, ce_config_light_files[i], NULL);
    ...
    static void ce_config_manager_init_movies(void)
    {
        for (size_t i = 0; i < CE_CONFIG_MOVIE_COUNT; ++i) {
            ce_config_manager->movies[i].reserve(4);
        }

        std::vector<char> path(ce_option_manager->ei_path->length + 32);
        ce_path_join(path.data(), path.size(), ce_option_manager->ei_path->str, ce_config_dir, "movie.ini", NULL);
            ...
            for (size_t i = 0; i < CE_CONFIG_MOVIE_COUNT; ++i) {
                ...
                if (NULL != line) {
                    ...
                    do {
                        name = ce_strsep(&pos, ",");
                        if ('\0' != name[0]) {
                            ce_config_manager->movies[i].push_back(name);
                        }
                    } while (NULL != pos);
                }
            }
    ...
    static void ce_config_manager_init_music(void)
    {
        for (size_t i = 0; i < CE_CONFIG_MUSIC_CHAPTER_COUNT; ++i) {
            for (size_t j = 0; j < CE_CONFIG_MUSIC_ITEM_COUNT; ++j) {
                ce_config_manager->music[i][j].reserve(4);
            }
        }

        std::vector<char> path(ce_option_manager->ei_path->length + 32);
        ce_path_join(path.data(), path.size(), ce_option_manager->ei_path->str, ce_resource_dir, "music.reg", NULL);

        memory_file_ptr_t mem_file = make_memory_file(path.data());
        if (mem_file) {
            ...
            for (size_t i = 0; i < CE_CONFIG_MUSIC_CHAPTER_COUNT; ++i) {
                for (size_t j = 0; j < CE_CONFIG_MUSIC_ITEM_COUNT; ++j) {
                    for (size_t k = 0; ; ++k) {
                        ...
                        ce_config_manager->music[i][j].push_back(ce_value_get_string(value));
                    }
                }
            }
    ...
    static void ce_config_manager_init_menus(void)
    {
        if (NULL != ce_resource_manager->menus) {
            memory_file_ptr_t mem_file = ce_res_ball_extract_mem_file_by_name(ce_resource_manager->menus, "menus.reg");
            assert(mem_file && "menus.reg required");
            ce_reg_file* reg_file = ce_reg_file_new(mem_file);
