...
namespace cursedearth
{
    ...
    {
        chunk->data = static_cast<unsigned char*>(malloc(block_size * block_count));
        chunk->next_block = 0;
    ...
    static void ce_alloc_portion_init(ce_alloc_portion* portion, size_t block_size)
    {
        ...
        portion->chunks = static_cast<ce_alloc_chunk*>(malloc(sizeof(ce_alloc_chunk) * portion->chunk_capacity));
    ...
    static void ce_alloc_portion_ensure_alloc_chunk(ce_alloc_portion* portion)
    {
        ...
        if (portion->chunk_count == portion->chunk_capacity) {
            ...
            portion->chunks = static_cast<ce_alloc_chunk*>(realloc(portion->chunks, sizeof(ce_alloc_chunk) * portion->chunk_capacity));
            portion->alloc_chunk = NULL;
            portion->dealloc_chunk = NULL;
        }
    ...
    void ce_alloc_init(void)
    {
        if (!ce_alloc_context.inited) {
            ...
            ce_alloc_context.portions = static_cast<ce_alloc_portion*>(malloc(sizeof(ce_alloc_portion) * ce_alloc_context.portion_count));
            for (size_t i = 0; i < ce_alloc_context.portion_count; ++i) {
    ...
    void* ce_alloc(size_t size)
    {
        ...
        void* ptr = size > CE_ALLOC_MAX_SMALL_OBJECT_SIZE ? malloc(size) :
            ce_alloc_portion_alloc(ce_alloc_context.portions + ce_alloc_get_offset(size) - 1);
