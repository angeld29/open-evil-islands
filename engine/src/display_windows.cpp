...
namespace cursedearth
{
    ...
    void ce_dmmng_change_display_settings(DEVMODE* dm, DWORD flags)
    {
        LONG code = ChangeDisplaySettingsEx(NULL, dm, NULL, flags, NULL);

        switch (code) {
        case DISP_CHANGE_BADDUALVIEW:
            ce_logging_error("displaymng: the settings change was unsuccessful because the system is DualView capable");
            break;
        ...
        case DISP_CHANGE_BADPARAM:
            ce_logging_error("displaymng: an invalid parameter was passed in; this can include an invalid flag or combination of flags");
            break;
        case DISP_CHANGE_FAILED:
            ce_logging_error("displaymng: the display driver failed the specified graphics mode");
            break;
        case DISP_CHANGE_NOTUPDATED:
            ce_logging_error("displaymng: unable to write settings to the registry");
            break;
        case DISP_CHANGE_RESTART:
            ce_logging_error("displaymng: the computer must be restarted for the graphics mode to work");
            break;
        }
    ...
    void ce_dmmng_enter(ce_displaymng* displaymng, size_t index, ce_display_rotation, ce_display_reflection)
    {
        ce_dmmng* dmmng = (ce_dmmng*)displaymng->impl;
        DEVMODE* mode = (DEVMODE*)dmmng->modes->items[index];
        ce_dmmng_change_display_settings(mode, CDS_FULLSCREEN);
    }

    void ce_dmmng_exit(ce_displaymng* displaymng)
    {
        ce_dmmng* dmmng = (ce_dmmng*)displaymng->impl;
