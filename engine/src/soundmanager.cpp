...
static void ce_sound_manager_create_instance(ce_event* event)
{
    ...
    if (NULL == mem_file) {
        char path[ce_option_manager->ei_path->length + strlen(sound_event->name) + 32];
        if (NULL == ce_path_find_special1(path, sizeof(path),
                ce_option_manager->ei_path->str, sound_event->name, ce_sound_dirs, ce_sound_exts)) {
...
static void ce_sound_manager_remove_instance(ce_event* event)
{
    ce_sound_event* sound_event = (ce_sound_event*)event->impl;
    ce_hash_remove(ce_sound_manager->sound_instances, sound_event->hash_key);
}

static void ce_sound_manager_state_instance(ce_event* event)
{
    ce_sound_event* sound_event = (ce_sound_event*)event->impl;
...
static void ce_sound_manager_query_instance(ce_hash_key hash_key)
{
    ce_sound_instance* sound_instance = ce_hash_find(ce_sound_manager->sound_instances, hash_key);
    if (NULL != sound_instance) {
...
void ce_sound_manager_init(void)
{
    ...
    ce_sound_manager->sound_bundles = ce_hash_new(32, ce_sound_bundle_del);
