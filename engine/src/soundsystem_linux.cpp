...
namespace cursedearth
{
    ...
    void ce_alsa_error_handler(const char* file, int line, const char* function, int code, const char* format, ...)
    {
        ce_logging_debug("alsa: error %d (%s:%d in %s)", code, file, line, function);
        va_list args;
        va_start(args, format);
        std::vector<char> buffer(strlen(format) + 16);
        snprintf(buffer.data(), buffer.size(), "alsa: %s", format);
        ce_logging_debug_va(buffer.data(), args);
        va_end(args);
    }
    ...
    int ce_alsa_set_params(void)
    {
        ...
        code = snd_pcm_sw_params_set_start_threshold(alsa->handle, swparams, (buffer_size / period_size) * period_size);
        if (code < 0) {
    ...
    {
        ce_sound_system_vtable vt = { sizeof(ce_alsa), ce_alsa_ctor, ce_alsa_dtor, ce_alsa_write };
        return vt;
    }
}
