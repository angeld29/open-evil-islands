...
/*
 *  Based on:
 *  1. DDS GIMP plugin (C) 2004-2008 Shawn Kirst <skirst@insightbb.com>,
...
static void compress_dxt1(unsigned char *dst, const unsigned char *src,
                          int w, int h, int dither, int alpha)
{
   ...
   for(y = 0; y < h; y += 4)
   {
      for(x = 0; x < w; x += 4)
      {
         ...
         encode_color_block(dst, block, dither, alpha);
         dst += 8;
      }
   }
}
...
void dxt_compress(unsigned char *dst, unsigned char *src, int format,
				unsigned int width, unsigned int height, int dither)
{
   assert(0 == (width & 3) && 0 == (height & 3)); // is mul4

   assert(CE_MMP_DXT1 == format);
   compress_dxt1(dst, src, width, height, dither, 1);
}
