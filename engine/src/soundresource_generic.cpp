...
{
    /**
     * @brief Ogg Vorbis (C) Xiph.Org Foundation
     */
    struct ce_vorbis
    {
        OggVorbis_File vf;
    ...
    size_t ce_vorbis_read_wrap(void* ptr, size_t size, size_t nmemb, void* datasource)
    {
        return ce_mem_file_read((ce_mem_file*)datasource, ptr, size, nmemb);
    }

    int ce_vorbis_seek_wrap(void* datasource, ogg_int64_t offset, int whence)
    {
        return ce_mem_file_seek((ce_mem_file*)datasource, offset, whence);
    }

    long ce_vorbis_tell_wrap(void* datasource)
    {
        return ce_mem_file_tell((ce_mem_file*)datasource);
    }

    bool ce_vorbis_test(ce_sound_probe* sound_probe)
    {
        char fourcc[4];
        if (4 != ce_mem_file_read(sound_probe->mem_file, fourcc, 1, 4) || 0 != memcmp(fourcc, "OggS", 4)) {
            return false;
        }
        ...
        OggVorbis_File vf;
        ov_callbacks cb = { ce_vorbis_read_wrap, ce_vorbis_seek_wrap, NULL, ce_vorbis_tell_wrap };
        if (0 == ov_test_callbacks(sound_probe->mem_file, &vf, NULL, 0, cb)) {
    ...
    bool ce_vorbis_ctor(ce_sound_resource* sound_resource, ce_sound_probe* sound_probe)
    {
    ...
    void ce_vorbis_dtor(ce_sound_resource* sound_resource)
    {
        ce_vorbis* vorbis = (ce_vorbis*)sound_resource->impl;
        ov_clear(&vorbis->vf);
    }

    bool ce_vorbis_decode(ce_sound_resource* sound_resource)
    {
        ce_vorbis* vorbis = (ce_vorbis*)sound_resource->impl;
    ...
    bool ce_vorbis_reset(ce_sound_resource* sound_resource)
    {
        ce_vorbis* vorbis = (ce_vorbis*)sound_resource->impl;
    ...
    /**
     * @brief FLAC: Free Lossless Audio Codec (C) Josh Coalson
     */
    typedef struct {
    ...
    ce_flac_bundle* ce_flac_bundle_new(void)
    {
        ce_flac_bundle* flac_bundle = (ce_flac_bundle*)ce_alloc_zero(sizeof(ce_flac_bundle));
        flac_bundle->decoder = FLAC__stream_decoder_new();
        return flac_bundle;
    }

    void ce_flac_bundle_del(ce_flac_bundle* flac_bundle)
    {
        if (NULL != flac_bundle) {
    ...
    const char* ce_flac_state_to_string(FLAC__StreamDecoderState state)
    {
        switch (state) {
    ...
    const char* ce_flac_get_state_string(const FLAC__StreamDecoder* decoder)
    {
        return ce_flac_state_to_string(FLAC__stream_decoder_get_state(decoder));
    }

    void ce_flac_error(const FLAC__StreamDecoder* decoder, const char* message)
    {
        ce_logging_error("flac: %s", message);
        ce_logging_error("flac: %s", ce_flac_get_state_string(decoder));
    }

    FLAC__StreamDecoderReadStatus ce_flac_read_callback(const FLAC__StreamDecoder*, FLAC__byte buffer[], size_t* bytes, void* client_data)
    {
        ce_flac_bundle* flac_bundle = (ce_flac_bundle*)client_data;
    ...
    FLAC__StreamDecoderSeekStatus ce_flac_seek_callback(const FLAC__StreamDecoder*, FLAC__uint64 absolute_byte_offset, void *client_data)
    {
        ce_flac_bundle* flac_bundle = (ce_flac_bundle*)client_data;
    ...
    FLAC__StreamDecoderTellStatus ce_flac_tell_callback(const FLAC__StreamDecoder*, FLAC__uint64* absolute_byte_offset, void* client_data)
    {
        ce_flac_bundle* flac_bundle = (ce_flac_bundle*)client_data;
    ...
    FLAC__StreamDecoderLengthStatus ce_flac_length_callback(const FLAC__StreamDecoder*, FLAC__uint64* stream_length, void* client_data)
    {
        ce_flac_bundle* flac_bundle = (ce_flac_bundle*)client_data;
    ...
    FLAC__bool ce_flac_eof_callback(const FLAC__StreamDecoder*, void* client_data)
    {
        ce_flac_bundle* flac_bundle = (ce_flac_bundle*)client_data;
        return ce_mem_file_eof(flac_bundle->mem_file) ? true : false;
    }

    FLAC__StreamDecoderWriteStatus ce_flac_write_callback(const FLAC__StreamDecoder*, const FLAC__Frame* frame, const FLAC__int32* const buffer[], void* client_data)
    {
        ce_flac_bundle* flac_bundle = (ce_flac_bundle*)client_data;
    ...
    void ce_flac_metadata_callback(const FLAC__StreamDecoder*, const FLAC__StreamMetadata* metadata, void* client_data)
    {
        ce_flac_bundle* flac_bundle = (ce_flac_bundle*)client_data;
    ...
    void ce_flac_error_callback(const FLAC__StreamDecoder*, FLAC__StreamDecoderErrorStatus status, void* /*client_data*/)
    {
    ...
    bool ce_flac_test(ce_sound_probe* sound_probe)
    {
        char four_cc[4];
        ...
        sound_probe->output_buffer_capacity = flac_bundle->sound_format.sample_size * flac_bundle->max_block_size;
    ...
    bool ce_flac_ctor(ce_sound_resource* sound_resource, ce_sound_probe* sound_probe)
    {
    ...
    void ce_flac_dtor(ce_sound_resource* sound_resource)
    {
        ce_flac* flac = (ce_flac*)sound_resource->impl;
        ce_flac_bundle_del(flac->bundle);
    }

    bool ce_flac_decode(ce_sound_resource* sound_resource)
    {
        ce_flac* flac = (ce_flac*)sound_resource->impl;
        ...
        sound_resource->output_buffer_size = flac->bundle->block_size * flac->bundle->sound_format.sample_size;
        return true;
    }

    bool ce_flac_reset(ce_sound_resource* sound_resource)
    {
        ce_flac* flac = (ce_flac*)sound_resource->impl;
    ...
    /**
     * @brief Waveform Audio File Format (C) Microsoft & IBM
     */
    typedef struct {
    ...
    bool ce_wave_test(ce_sound_probe* sound_probe)
    {
        ce_wave_header wave_header;
    ...
    bool ce_wave_ctor(ce_sound_resource* sound_resource, ce_sound_probe* sound_probe)
    {
    ...
    bool ce_wave_decode(ce_sound_resource* sound_resource)
    {
        ce_wave* wave = (ce_wave*)sound_resource->impl;
        ...
        case CE_WAVE_FORMAT_IMA_ADPCM:
            ...
            ce_mem_file_read(sound_resource->mem_file, sound_resource->input_buffer, 1, sound_resource->input_buffer_capacity);

            ce_wave_ima_adpcm_decode(sound_resource->output_buffer, sound_resource->input_buffer, &wave->wave_header);
            return true;
        }
    ...
    bool ce_wave_reset(ce_sound_resource* sound_resource)
    {
        ce_wave* wave = (ce_wave*)sound_resource->impl;
    ...
    /**
     * @brief MAD: MPEG Audio Decoder (C) Underbit Technologies, Inc.
    ...
    bool ce_mad_test(ce_sound_probe* sound_probe)
    {
        unsigned char* buffer = (unsigned char*)ce_alloc(CE_MAD_INPUT_BUFFER_CAPACITY);
    ...
    void ce_mad_error(ce_sound_resource* sound_resource, ce_logging_level level)
    {
    ...
    bool ce_mad_input(ce_sound_resource* sound_resource)
    {
        ce_mad* mad = (ce_mad*)sound_resource->impl;
        ...
        mad_stream_buffer(&mad->stream, (const unsigned char*)sound_resource->input_buffer, remaining + size);
    ...
    int16_t ce_mad_scale(mad_fixed_t sample, ce_mad_dither* dither, ce_mad_stats* stats)
    {
        // noise shape
    ...
    bool ce_mad_decode(ce_sound_resource* sound_resource)
    {
        ...
        while (-1 == mad_frame_decode(&mad->frame, &mad->stream)) {
            if (MAD_ERROR_BUFLEN == mad->stream.error || MAD_ERROR_BUFPTR == mad->stream.error) {
    ...
    void ce_mad_dtor(ce_sound_resource* sound_resource)
    {
        ce_mad* mad = (ce_mad*)sound_resource->impl;
        ce_mad_clean(mad);
    }

    bool ce_mad_reset(ce_sound_resource* sound_resource)
    {
        ce_mad* mad = (ce_mad*)sound_resource->impl;
        ce_mad_clean(mad);
        ce_mad_init(mad);
    ...
    bool ce_bink_ctor(ce_sound_resource* sound_resource, ce_sound_probe* sound_probe)
    {
        ce_bink* bink = (ce_bink*)sound_resource->impl;

        memcpy(&bink->header, sound_probe->buffer, sizeof(ce_bink_header));
        bink->indices = (ce_bink_index*)ce_alloc(sizeof(ce_bink_index) * bink->header.frame_count);
    ...
    void ce_bink_dtor(ce_sound_resource* sound_resource)
    {
        ce_bink* bink = (ce_bink*)sound_resource->impl;
        ce_free(bink->indices, sizeof(ce_bink_index) * bink->header.frame_count);
    ...
    bool ce_bink_decode(ce_sound_resource* sound_resource)
    {
        ce_bink* bink = (ce_bink*)sound_resource->impl;
    ...
    bool ce_bink_reset(ce_sound_resource* sound_resource)
    {
        ce_bink* bink = (ce_bink*)sound_resource->impl;
        bink->frame_index = 0;
